...
class _CommunicationProfileTab extends StatefulWidget {
  ...
  @override
  State<_CommunicationProfileTab> createState() =>
      _CommunicationProfileTabState();
}
...
class _CommunicationProfileTabState extends State<_CommunicationProfileTab> {
  ...
  Future<void> _save() async {
    await CareCoordinationStore.saveProfile(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Perfil funcional salvo localmente.')),
    );
  }
              ...
              child: Column(
                children: [
                  _field(
                    ...
                    'Ex.: símbolos, fala, gestos, olhar, dispositivo',
                  ),
                  _field(
                    _access,
                    'Como acessa o dispositivo?',
                    Icons.touch_app_outlined,
                    'Ex.: toque, acionador, teclado, ajuda',
                  ),
                  _field(
                    _facilitators,
                    'O que facilita a participação?',
                    Icons.thumb_up_alt_outlined,
                    'Ex.: antecipação visual, duas opções, tempo de resposta',
                  ),
                  _field(
                    _avoid,
                    'O que devemos evitar?',
                    Icons.block_outlined,
                    'Ex.: pressa, excesso de estímulos, tocar sem avisar',
                  ),
                  _field(
                    _contingency,
                    'Se o dispositivo não estiver disponível',
                    Icons.health_and_safety_outlined,
                    'Ex.: cartões impressos, gestos combinados, parceiro de comunicação',
                  ),
                  _field(
                    _partners,
                    'Parceiros que já conhecem este perfil',
                    Icons.groups_outlined,
                    'Ex.: família, escola, clínica',
                  ),
                  ListTile(
...
class _CommunicationPlanTabState extends State<_CommunicationPlanTab> {
  ...
  Future<void> _save() async {
    if (_title.text.trim().isEmpty || _goal.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Informe um título e um objetivo funcional.'),
        ),
      );
      return;
    }
...
class _AgendaTabState extends State<_AgendaTab> {
  ...
  Future<void> _save() async {
    if (_title.text.trim().isEmpty || _professional.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Informe o compromisso e o profissional.'),
        ),
      );
      return;
    }
              ...
              child: Column(
                children: [
                  ...
                  ListTile(
                    ...
                    subtitle: Text(
                      '${_date(_startsAt)} às ${_startsAt.hour.toString().padLeft(2, '0')}:${_startsAt.minute.toString().padLeft(2, '0')}',
                    ),
                    onTap: _pickDateTime,
                  ),
                  SwitchListTile(
